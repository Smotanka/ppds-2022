...
class MySyncServer(BaseHTTPRequestHandler):
    """
...
def get_images(url: str, num: int) -> list[str]:
    """
    Synchronously create request to a given url and returns content of the requests
    :param url: string: - url for request
...
def run(server_class=HTTPServer, handler_class=BaseHTTPRequestHandler):
    ...
    :return: None
    """
    server_address = ('', 8000)  # we want to run on localhost
...
if __name__ == '__main__':
    run(handler_class=MySyncServer)
